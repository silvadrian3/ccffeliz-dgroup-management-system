app.controller('profileCtrl', ['$scope', '$http', '$cookies', '$cookieStore', '$filter', function ($scope, $http, $cookies, $cookieStore, $filter) {
    ...
    $scope.updateProfile = function () {
        ...
        $http.put(baselocation + "/api/v1/users.php?m=" + el + "&user_id=" + ui, params).success(function (response) {
...
app.controller('changepasswordCtrl', ['$scope', '$http', '$cookies', '$cookieStore', '$filter', function ($scope, $http, $cookies, $cookieStore, $filter) {
    ...
    $scope.checkpassword = function () {
        ...
        if (currentpassword !== undefined && currentpassword !== "") {
            $http.get(baselocation + "/api/v1/password.php?m=" + el + "&action=checkPassword&password=" + currentpassword + "&user_id=" + ui).success(function (response) {
    ...
    $scope.changepassword = function () {
        ...
        if (!$scope.incorrectpassword && !$scope.passwordmismatch) {
            ...
            $http.put(baselocation + "/api/v1/password.php?m=" + el + "&action=changePassword&user_id=" + ui, params).success(function (response) {
                if (response[0].result) {
                    var params = {
                            user_id: ui,
                            module: 'Profile',
